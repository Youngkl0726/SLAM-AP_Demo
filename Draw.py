        ...
        for line in (ap_pos[i]):
            ...
            for u in range(top, bottom):
                for v in range(left, right):
                    ...
                    x = (v-cx)*z*invfx
                    y = (u-cy)*z*invfy
                    x3dc = [[] for k in range(4)]
                    x3dc[0].append(x)
                    x3dc[1].append(y)
                    x3dc[2].append(z)
                    x3dc[3].append(1.0)
                    mat_3dc = np.mat(x3dc)
                    mat_pos = mat_rr.dot(mat_3dc)
                    list_pos = mat_pos.tolist()
                    # print list_pos.shape
                    ...
                    # cnt += 1.0
                    total_x -= list_pos[0][0]
                    total_y -= list_pos[2][0]
                    cnt += 1.0
            # if i > 295 and i < 310:
            #     print "cnt is : ", cnt
            # print total_y.shape
